    ...
    def load_file(self, file: str = None):
        ...
        with open(file, 'r', encoding='UTF-8') as M3U_Playlist:
            for file in M3U_Playlist.readlines():
                file.removesuffix('\n')
                if file[0] != '#' and file[-4:] == '.mp3' and os.path.exists(file) and self._musicData.exists_file(file): 
...
mode : 2 play de la cançó però no print
        ...
        for song_id in self._songs:
            title = self._musicData.get_attribute(song_id, 'title')
            print(f"\nPlaying [{title}]")
            beg = time()
            self._musicPlayer.play_song( uuid=song_id,
                                         mode=mode )
            print(f"Finished [{title}] in {time() - beg} seconds")
